...
public interface QLNDuserRepository extends JpaRepository<QLNDUser, UUID> {
      ...
      @Query(value = "SELECT u.FullName AS fullName, u.Password AS password, g.Name AS Name FROM QLKV_UAT.dbo.QLNDUser u JOIN QLKV_UAT.dbo.QLNDUserGroup ug ON u.Id = ug.UserId JOIN QLKV_UAT.dbo.QLNDGroup g ON ug.GroupId = g.Id WHERE u.LoginName = :loginName",nativeQuery = true)
